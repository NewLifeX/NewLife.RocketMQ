...
> 当前客户端版本：3.0.x  
> 协议版本声明：`MQVersion.V4_9_7`（默认，可配置）  
...
NewLife.RocketMQ 是新生命团队开发的**纯托管轻量级 RocketMQ 客户端**，同时支持 RocketMQ Remoting 协议（TCP 私有协议）和 **gRPC Proxy 协议**（RocketMQ 5.x），不依赖 Java 客户端或第三方 gRPC / Protobuf 库。
...
- 内置轻量级 Protobuf 编解码器（ProtoWriter/ProtoReader，无外部依赖）
- 支持 SSL/TLS 加密传输（`SslProtocol` + `Certificate`）
- 支持多云厂商签名认证（阿里云/华为云/腾讯云/Apache ACL），统一由 `ICloudProvider` 适配
- 统一 HMAC-SHA1 签名机制（`ClusterClient.SetSignature`）
- 单连接复用，Opaque 请求-响应匹配
- 支持外部代理映射（`ExternalBroker` 属性）
...
| **公网版消费者状态** | ?? | 阿里云公网版返回的 `brokerName` 与 NameServer 路由中的不一致，需特殊处理（已有容错代码） |
...
| **gRPC Proxy 模式** | ?? | 阿里云推荐使用 gRPC 方式接入，客户端已支持 gRPC 协议，待验证兼容性 |
...
### 6.4 gRPC 服务方法（RocketMQ 5.x Proxy）
