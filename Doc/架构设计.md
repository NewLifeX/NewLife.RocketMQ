...
| Consumer | ChangeInvisibleTimeAsync | `Task ChangeInvisibleTimeAsync(MessageExt msg, Int64 invisibleTime, Boolean incrementReconsumeTimes = true)` | msg, invisibleTime, incrementReconsumeTimes | Task | 修改不可见时间（可选不递增重试次数） |
| GrpcPushConsumer | OnMessage | `Func<GrpcMessage, Task<Boolean>>` | 委托 | — | gRPC Push 消费回调，内置长轮询线程自动 Ack（5.x） |
...
### 3.6 IMessageCompressor 可插拔压缩接口

| 接口 | 方法 | 签名 | 入参 | 出参 | 说明 |
|------|------|------|------|------|------|
| IMessageCompressor | Compress | `Byte[] Compress(Byte[] data)` | data: 原始数据 | 压缩后字节数组 | 压缩消息体 |
| IMessageCompressor | Decompress | `Byte[] Decompress(Byte[] data)` | data: 压缩数据 | 解压后字节数组 | 解压消息体 |
| MessageCompressorRegistry | Register | `void Register(Int32 compressType, IMessageCompressor compressor)` | compressType: SysFlag 压缩类型, compressor: 压缩器实例 | 无 | 注册压缩算法 |
| MessageCompressorRegistry | Get | `IMessageCompressor Get(Int32 compressType)` | compressType: 压缩类型 | 压缩器实例 | 获取压缩器 |

SysFlag 压缩类型对应关系（bits 8~9）：

| compressType 值 | 算法 | 实现位置 |
|:-----------:|------|:--------:|
| 0 / 3 | ZLIB/DEFLATE | 内置 `ZlibMessageCompressor`（主库，零依赖） |
| 1 | LZ4 | `LZ4MessageCompressor`（扩展包 `NewLife.RocketMQ.Extensions`） |
| 2 | ZSTD | `ZstdMessageCompressor`（扩展包 `NewLife.RocketMQ.Extensions`） |
...
| 可插拔压缩（扩展） | IMessageCompressor 接口 + NewLife.RocketMQ.Extensions 可选包 | 保持主库零依赖，同时支持 LZ4/ZSTD 高性能算法 |
...
| 可插拔压缩接口 | IMessageCompressor 接口 + 扩展项目 | 直接引入外部 NuGet 依赖到主库 | 主库保持零依赖；LZ4/ZSTD 用户按需安装扩展包，不影响未使用压缩的用户 |
...
| 批次 7 | T045~T051：F052~F056+F058（5.x 协议增强，F057 单独列入批次 8） | ✅ |

### 待实现批次

| 批次 | 任务范围 | 状态 |
|------|---------|:----:|
| 批次 8 | F057 Lite Topic：TopicMessageType 枚举 + CreateTopic 新增 topicMessageType 参数 | ✅ |
...
| LZ4/ZSTD 扩展包外部依赖 | NewLife.RocketMQ.Extensions 引入第三方压缩库，包体增大 | 主库保持零依赖；扩展包为可选依赖，用户按需安装 |
| ACL 2.0 Broker 版本兼容 | ACL 2.0 请求头在旧版本 Broker 可能被拒绝 | 新字段加入前检测 Broker 版本；旧客户端默认不加入该字段，保持向后兼容 |
...
| TopicMessageType.cs | ~30 | Topic 消息类型枚举（Normal/Fifo/Delay/Transaction/Lite，F057） |
...
> 注：F052~F058 新增单元测试 42 个，分布在 `CompressionTests`、`ProtoTests`、`SupportApacheAclTest`、`PopConsumeTests`、`MessageTests`、`GrpcPushConsumerTests` 共 6 个文件中；F057 新增 `LiteTopicTests`（7 个）和 `LiteTopicIntegrationTests`（3 个）。当前全量测试：**523 通过，0 失败，6 跳过（Aliyun 云服务依赖）**，总计 529 个测试用例。
...
| **E2E 集成测试** | OrderConsumeIntegrationTests, PopConsumeIntegrationTests, BatchAckIntegrationTests, ConsumerRetryDLQIntegrationTests, BroadcastConsumeIntegrationTests, MultiTopicIntegrationTests, SQL92FilterIntegrationTests, VipChannelIntegrationTests, MessageTraceIntegrationTests | 顺序消费、Pop+Ack、批量Ack、重试/DLQ、广播消费、多Topic、SQL92过滤、VIP通道、消息轨迹端到端 |
