...
- **验收条件**（AC）：
  ...
  - [x] IPv4/IPv6 地址自动识别和解析（BornHost：SysFlag 第 4 位 0x10；StoreHost：第 5 位 0x20）
  - [x] 批量消息解码（DecodeBatch，INNER_BATCH_FLAG=0x80 标识）
...
- **描述**：RocketMQ 5.5.0 引入 Lite Topic（RIP-83），专为 AI Agent 等大量临时队列场景设计，支持按需动态创建数百万级轻量 Topic，无需提前运维创建。
- **用户故事**：作为 AI 应用开发者，我希望为每个 AI Agent 任务按需创建临时 Topic，以便实现任务级消息隔离而无需运维介入。
- **验收条件**（AC）：
  - [x] 创建 Topic 时支持 `TopicMessageType.Lite` 属性（`TopicMessageType` 枚举 + `CreateTopic` 新增 `topicMessageType` 参数）
  - [x] 路由查询透明支持（Broker 5.5.0+ 会在路由中返回 Lite Topic，客户端无需特殊处理）
  - [x] 文档说明 Lite Topic 仅支持 RocketMQ 5.5.0+ Broker（旧版 Broker 忽略 `topicMessageType` 字段，以 Normal 类型创建）
- **优先级**：Could
- **完成状态**：✅ 已完成（注：需 RocketMQ 5.5.0+ Broker 具备完整 Lite Topic 特性；旧版 Broker 忽略该字段）

### 3.16 gRPC PushConsumer（F058）
...
### 迭代 7：5.x 协议增强（Should/Could）— ✅ 全部完成

| 编号 | 功能点 | 验收条件 | 前置依赖 | 完成状态 |
|------|--------|---------|---------|:-------:|
| F052 | 消息压缩可插拔接口 | IMessageCompressor 接口 + ZLIB 内置 + 扩展项目 | F019 | ✅ |
| F053 | 优先级消息 | gRPC Priority 字段序列化/反序列化 | F035 | ✅ |
| F054 | ACL 2.0 权限模型 | 请求头注入 + 兼容 ACL 1.x | F032 | ✅ |
| F055 | ChangeInvisibleDuration 不递增重试次数 | `incrementReconsumeTimes` 参数 | F025 | ✅ |
| F056 | LMQ 轻量消息队列 | 消息属性设置/读取辅助方法 | F005 | ✅ |
| F057 | Lite Topic | Broker 5.5.0+ 动态轻量 Topic（Could） | F003 | ✅ |
| F058 | gRPC PushConsumer | 长轮询线程 + 回调式 API | F035 | ✅ |
...
| F052~F056, F057, F058 | 5.x 协议增强（迭代 7 全部） | 可插拔压缩/优先级/ACL 2.0/Pop 不递增重试/LMQ/Lite Topic/gRPC Push | ✅ 通过 | F052 扩展包 LZ4/ZSTD 为可选；F057 在 5.5.0+ Broker 上具备完整 Lite 特性 |
...
| PopConsumeTests | 8 | Pop 消费异常处理与请求码（含 F055 incrementReconsumeTimes） | ✅ 通过 |
...
| SupportApacheAclTest | 4 | ACL 2.0 请求头注入（F054） | ✅ 通过 |
| MessageTests（LMQ 部分） | 5 | LMQ 轻量队列属性设置/读取（F056） | ✅ 通过 |
| ProtoTests（Priority 部分） | 5 | gRPC 优先级字段序列化（F053） | ✅ 通过 |
| GrpcPushConsumerTests | 8 | gRPC PushConsumer 回调与 Ack 路径（F058） | ✅ 通过 |
| CompressionTests（扩展） | 5 | 可插拔压缩器注册/ZLIB 编解码（F052） | ✅ 通过 |
| LiteTopicTests | 7 | Lite Topic 枚举定义与 ToString（F057） | ✅ 通过 |
| **合计** | **523** | — | **✅ 全部通过（6跳过 Aliyun 云服务依赖）** |

> 上述 523 单元测试含集成测试（需要本地 RocketMQ 服务器），截至最新测试运行：**523 通过，0 失败，6 跳过（Aliyun 云服务依赖）**，总计 529 个测试用例。

**新增端到端集成测试（2025-07）：**

| 测试文件 | 测试数 | 覆盖场景 |
|---------|:------:|---------|
| OrderConsumeIntegrationTests | 2 | 顺序消费 E2E、LockBatchMQ/UnlockBatchMQ |
| PopConsumeIntegrationTests | 2 | Pop 拉取+Ack、修改不可见时间 |
| BatchAckIntegrationTests | 2 | 批量 Ack 三条消息、空条目返回 0 |
| ConsumerRetryDLQIntegrationTests | 2 | 重试机制验证、关闭重试行为 |
| BroadcastConsumeIntegrationTests | 1 | 广播模式双消费者各自收全量消息 |
| MultiTopicIntegrationTests | 1 | 单消费者订阅多 Topic |
| SQL92FilterIntegrationTests | 1 | SQL92 属性过滤（需 Broker 开启 enablePropertyFilter） |
| MessageTraceIntegrationTests | 2 | 生产者/消费者轨迹记录到达 RMQ_SYS_TRACE_TOPIC |
| **新增小计** | **15** | — |
...
| IMessageCompressor | 可插拔消息压缩接口，支持 ZLIB（内置）/ LZ4 / ZSTD（扩展包 NewLife.RocketMQ.Extensions）等算法 |
| 优先级消息 | RocketMQ 5.4.0（RIP-80）引入，通过 gRPC SystemProperties.Priority 字段（1~16）设置消息投递优先级 |
| ACL 2.0 | RocketMQ 5.3+ 的资源级权限模型，在请求头附加 aclEnabled / resourceType / resourceName 字段 |
| LMQ | Lite Message Queue，RocketMQ 4.9+ 引入，适合物联网海量设备独立轻量队列场景 |
| Lite Topic | RocketMQ 5.5.0（RIP-83）引入，支持按需动态创建数百万级轻量 Topic，适合 AI Agent 等场景 |
| gRPC PushConsumer | 5.x gRPC 协议消费者类型，内置长轮询线程，通过 OnMessage 回调驱动消费，区别于 SimpleConsumer |
| ReconsumeTimes | 消息重试次数，Pop 消费模式下 ChangeInvisibleDuration 可选择不递增该计数器（RocketMQ 5.5.0+） |
