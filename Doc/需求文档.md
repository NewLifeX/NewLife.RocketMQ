...
- **验收条件**（AC）：
  - [ ] 创建 Topic 时支持 `TopicType=LITE` 属性
  - [ ] Broker API 支持轻量 Topic 的路由查询
  - [ ] 文档说明 Lite Topic 仅支持 RocketMQ 5.5.0+ Broker
- **优先级**：Could（暂不排入迭代计划，记录需求备查）
- **完成状态**：⚠️ 待实现
...
### 迭代 7：5.x 协议增强（Should/Could）— ⚠️ 待实现

| 编号 | 功能点 | 验收条件 | 前置依赖 | 完成状态 |
|------|--------|---------|---------|:-------:|
| F052 | 消息压缩可插拔接口 | IMessageCompressor 接口 + ZLIB 内置 + 扩展项目 | F019 | ⚠️ |
| F053 | 优先级消息 | gRPC Priority 字段序列化/反序列化 | F035 | ⚠️ |
| F054 | ACL 2.0 权限模型 | 请求头注入 + 兼容 ACL 1.x | F032 | ⚠️ |
| F055 | ChangeInvisibleDuration 不递增重试次数 | `incrementReconsumeTimes` 参数 | F025 | ⚠️ |
| F056 | LMQ 轻量消息队列 | 消息属性设置/读取辅助方法 | F005 | ⚠️ |
| F057 | Lite Topic | Broker 5.5.0+ 动态轻量 Topic（Could） | F003 | ⚠️ |
| F058 | gRPC PushConsumer | 长轮询线程 + 回调式 API | F035 | ⚠️ |
...
> 另有 125 个集成测试（含新增端到端测试），需要本地 RocketMQ 服务器。截至最新测试运行：**471 通过，0 失败，6 跳过（Aliyun 云服务依赖）**，总计 477 个测试用例。
